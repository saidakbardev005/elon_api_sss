...
def load_db_data():
    """
    1) announcements жадvalidан: pick_up_address, shipping_address, price олади.
       Колонкалар номини кичик ҳарф+strip() қилиб тозалайди, key fuzzy matching.
    2) driver_locations, my_autos, users жадвалларини ҳам DataFrame га олади.
    """
    ann = fetch_table("announcements")

    # колонка номларини барча бош/охир бўшлиқдан тозаб, кичик ҳарфга ўтказамиз
    ann.columns = [c.strip().lower() for c in ann.columns]

    # pick ва ship колонкаларини автоматик топиш
    pick_col  = next((c for c in ann.columns if "pick" in c and "address" in c), None)
    ship_col  = next((c for c in ann.columns if "ship" in c), None)
    ...
    # фақат шу учта колонкани олиб, жойни тозалаш
